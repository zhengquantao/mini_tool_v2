...
def build_html(factor_path, turbine, plot_power_df, *args, **kwargs):
    ...
    file_name = random_name(turbine, "赛马排行")
    ...
    headers = ["排行", "风机号", "能效系数(%)", "能量可利用率(%)", "实际发电量(kw/h)", "理论发电量(kw/h)"]
    rows = [[idx] + row for idx, row in enumerate(plot_power_df.values, start=1)]
    table.add(headers, rows)
    table.set_global_opts(
        title_opts=opts.ComponentTitleOpts(title="赛马排行")
    )
