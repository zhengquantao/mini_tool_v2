...
from models.bin_analysis.quant.cloud.wind_base_tool import rotor_speed_label
...
def main():
    ...
    # *** ---------- 1 基础设置和参数初始化 ----------
    scada_dir = "D:/VSCode_space/data/shapa/SCADA/"
    scada_files = os.listdir(scada_dir)

    # 理论功率曲线数据
    data_dir = "D:/VSCode_space/data/shapa/"
    theory_curve = pd.read_excel(os.path.join(data_dir,"power_curve_6.45.xlsx"))
    theory_curve.columns = [wind_speed_label, 'theory_power']
    ...
    for scada_file in scada_files:
        ...
        # ** 3.3 字段名称转换 **
        data = data[["real_time", "wtid", "grWindSpeed", "grGeneratorSpeed", "grRotorSpeed1",
                     "grPitchAngle1A", "grPitchAngle2A", "grPitchAngle3A",
                    "grNacellePositionToNorth", "grWindDirction", "grAirDensity", "grNacelleTemperture", "grGridActivePower"]]
        
        # "power",
        data.columns = ["real_time", "turbine_code", wind_speed_label, gen_speed_label, rotor_speed_label,
        ...
        data.loc[:, "power"] = data.loc[:, "power"].apply(lambda x: locale.atof(str(x)))

        data.loc[:, gen_speed_label] = data.loc[:, gen_speed_label].apply(lambda x: locale.atof(str(x)))
        ...
        if tsr_label not in norm_data.columns.to_list():
            tsr_calc(norm_data, gear_ratio, rotor_radius)
