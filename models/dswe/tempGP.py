...
class TempGP(object):
    ...
    def __init__(self, opt_method='L-BFGS-B', limit_memory=5000, fast_computation=True,
                 optim_control={'batch_size': 100,
                                'learning_rate': 0.05,
                                'max_iter': 5000,
                                'tol': 1e-6,
                                'beta1': 0.9,
        ...
        if fast_computation:
            ...
            if not set(
                    ['batch_size', 'learning_rate', 'max_iter', 'tol', 'beta1', 'beta2', 'epsilon', 'logfile']) == set(
                    list(optim_control.keys())):
                raise ValueError(
    ...
    def TempGPfit(self, X_train, y_train, T_train=[]):
    ...
    def update(self, X_update, y_update, T_update=[], replace=True, update_model_F=False):
        ...
        if update_model_F:
            ...
            self.residual = self.y_train - predict_GP(
                self.model_F['X_train'], self.model_F['weighted_y'], self.X_train,
                self.optim_result['estimated_params'])
