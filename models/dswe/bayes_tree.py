...
class BayesTreePowerCurve(object):
    ...
    def __init__(self, n_trees=200):
        try:
            ...
            print(
                "To run this model, please install the bartpy python library. You can see more details about it on the installation page of DSWE package.")
    ...
    def fit(self, X_train, y_train):
        ...
        if not (isinstance(X_train, list) or isinstance(X_train, pd.DataFrame) or isinstance(X_train,
                                                                                             pd.Series) or isinstance(
                X_train, np.ndarray)):
            raise ValueError(
                "The X_train should be either a list or numpy array or dataframe.")

        if not (isinstance(y_train, list) or isinstance(y_train, np.ndarray)) or isinstance(y_train,
                                                                                            pd.Series) or isinstance(
    ...
    def predict(self, X_test):
        ...
        if not (isinstance(X_test, list) or isinstance(X_test, pd.DataFrame) or isinstance(X_test,
                                                                                           pd.Series) or isinstance(
                X_test, np.ndarray)):
            raise ValueError(
