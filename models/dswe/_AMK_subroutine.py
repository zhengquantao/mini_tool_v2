...
def compute_weights(X_train, testpoint, bw, n_multi_cov, fixed_cov, cir_cov=None):
    ...
    if n_multi_cov == 'all':
        ...
        for i in range(cov_combination.shape[1]):
            ...
            for j in cov_combination[:, i]:
                if cir_cov and j in cir_cov:
                    cov_kernel = compute_von_mises_kernel(
                        X_train[:, j], testpoint[j], bw[j])
                    cov_kernel[np.isnan(cov_kernel)] = 0
                    if cov_kernel.sum() != 0:
                        kernel = kernel * cov_kernel
                else:
...
def compute_bandwidth(X_train, y_train, cir_cov):
    ...
    if not isinstance(cir_cov, type(None)):
        if ~np.isnan(np.array(cir_cov)).any():
            for circ in cir_cov:
                bandwidth[i] = (bandwidth[circ] * np.pi) / 180.
                bandwidth[i] = 1 / (bandwidth[circ] ** 2)
...
def compute_pred(X_train, y_train, X_test, bandwidth, n_multi_cov, fixed_cov, cir_cov):
    ...
    for i in range(len(pred)):
        ...
        pred[i] = np.matmul(np.sum(weights, axis=1),
                            y_train) / weights.shape[1]
