...
from .power_curve_tool import actual_power_label, theory_power_label
...
def aep_calc(data, power_curve, turbine_code, clean_percentage=None):
    """
    电量计算
    :param data: 输入数据时序集
    ...
    for i in range(1, len(power_curve)):
        ...
        iec_power_high, iec_power_low = power_curve.loc[i, actual_power_label], power_curve.loc[
            i - 1, actual_power_label]
        pdf_power_high, pdf_power_low = power_curve.loc[i, theory_power_label], power_curve.loc[
            i - 1, theory_power_label]
